...
class Die:
    ...
    def __post_init__(self) -> None:
        ...
        if self.min_tail < 0:
            raise ValueError(f"min_tail cannot be negative, got {self.min_tail}")
    ...
    def matches_clr(self, clr: float, tolerance: float = 0.01) -> bool:
        """
...
class Bender:
    ...
    def __post_init__(self) -> None:
        """Validate numeric fields are positive."""
        if self.min_grip <= 0:
            raise ValueError(f"min_grip must be positive, got {self.min_grip}")
    ...
    def find_die_for_clr(self, clr: float, tolerance: float = 0.01) -> Die | None:
